...
export class RepBudget extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      customersData: new AspNetData.createStore({
        key: "name",
        loadUrl: `${API_ENDPOINT}/RepCustomers`,
      }),
      accountsData: new AspNetData.createStore({
        key: "id",
        loadUrl: `${API_ENDPOINT}/RepAccounts`,
      }),
      repNamesData: [],
      productsData: [],
      initiativesData: [],
  ...
  getRepNames() {
    this.apiService
      .sendRequest(`${API_ENDPOINT}/RepNamesSelect`)
      .then((res) => this.setState({ repNamesData: res }));
  }

  geStatuses() {
    this.apiService
      .sendRequest(`${API_ENDPOINT}/RepStatuses`)
      .then((res) => this.setState({ statusesData: res }));
  }

  getEventTypes() {
    this.apiService
      .sendRequest(`${API_ENDPOINT}/RepEventTypes`)
      .then((res) => this.setState({ eventTypesData: res }));
  }

  getCustomerTypes() {
    this.apiService
      .sendRequest(`${API_ENDPOINT}/RepCustTypes`)
      .then((res) => this.setState({ custTypesData: res }));
  }

  getProducts() {
    this.apiService
      .sendRequest(`${API_ENDPOINT}/RepProducts`)
  ...
  getInitiatives() {
    this.apiService
      .sendRequest(`${API_ENDPOINT}/RepInitiatives`)
  ...
  calculateFilterExpression(filterValue, selectedFilterOperation, target) {
    ...
    return (rowData) =>
      (rowData.AssignedEmployee || []).indexOf(filterValue) !== -1;
  }
      ...
      <div className="section">
        <Container>
          <h2 className="section__title">
            Budget Representative / Budget Représentant
          </h2>
        </Container>
          ...
          >
            ...
            <Column
              ...
              calculateDisplayValue={(rowData) => {
                return (
                  repNamesData.find(
                    (i) => i.sales_Area_Code === rowData.sales_Area_Code
                  )?.employee_Name || rowData.rep_Employee_Name
                );
              }}
            ...
            <Toolbar>
              ...
              <Item>
                <Button
                  icon={isContainerFluid ? "panelright" : "panelleft"}
